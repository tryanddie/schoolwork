...
public class ControlBookController extends Application implements Initializable {
    ...
    void insert(ActionEvent event) {
        InsertController test = new InsertController ();
        try {
    ...
    public void start(Stage primaryStage) throws Exception {
        try {
            AnchorPane root = FXMLLoader.load(getClass().getResource("/fxml/Controllbook.fxml"));
    ...
    public int refresh(String sql)throws SQLException{
        ...
        while(rs.next()) {
            ...
            delete.setOnAction((ActionEvent e)->{
                try {
                    state.executeUpdate("delete from Book where bookname ='"+contain+"'");
                    String info = "删除成功";
                    Alert alert = new Alert(Alert.AlertType.INFORMATION, info, new ButtonType("确定", ButtonBar.ButtonData.YES));
                    alert.setHeaderText(null);
                    alert.setTitle("提示");
